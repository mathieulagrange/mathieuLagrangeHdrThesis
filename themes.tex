...
Pour le traitement du son, on s'accorde pour considérer que de nombreux éléments structurants sont communément utilisés par le sah pour inférer une représentation informative de la scène sonore auquel il est soumis. Pour les besoins de l'exemple, on représentera ici la sortie de la cochlée comme un spectrogramme\marginnote{MISSING INTRODUCTION DU SPECTROGRAMME FIGURE}, ce qui est, du point de vue physiologique, une approximation très grossière, mais conceptuellement suffisante pour notre propos\marginnote{Pour une présentation des différents modèles de référence, voir \cite{lyon2017human}}. La tâche du sah consiste alors à associer certains éléments du plan temps/fréquence à des sources données, et ce en fonction de certains critères.  On citera, dans l'ordre d'importance, l'harmonicité, la synchronicité, la proximité en fréquence, etc. L'importance de ces critères peut être modulée dans une certaine mesure par des processus descendants comme l'attention ou encore la pratique\marginnote{Helmhotlz se disait capable de dissocier les différentes harmoniques d'une note de violon grâce à l'écoute répétée du son de ses résonateurs. MISSING FIGURE}.
...
Séduit par l'aspect neuro-inspiré et la grande diversité algorithmique des approches CASA, j'ai  poursuivi pendant quelques années cet effort de recherche en proposant plusieurs méthodes basés sur des algorithmes de regroupement d'objets, \og clustering \fg en Anglais.
