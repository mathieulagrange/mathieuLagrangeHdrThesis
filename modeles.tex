...
\begin{enumerate}
  ...
  \item\textbf{expressivité} : un modèle expressif est capable de mettre à disposition des mécanismes de manipulation tels que la modification du modèle produise une modification cohérente de la perception du signal résultant. Ce critère peut s'exprimer formellement de la manière suivante : $\mathrm{Pe}(\mathrm{Sy}(X')) \sim \mathrm{Pe}(x')$, $X'$ désigne une version du modèle $X$ modifié avec des opérateurs simples, et $x'$ désigne une version modifiée du signal $x$ à partir d'opérateurs complexes: étirement, transposition, etc.
...
\end{eqnarray}
Cette transformée est donc inversible, mais n'est pas à échantillonnage critique,~\footnote{Une transformée est dite à échantillonnage critique quand un signal nécessite le même nombre de bits pour être représenté dans le domaine transformée que dans celui d'origine.} car $x\in \mathbb{R}$  et $X \in \mathbb{C}$.
...
\begin{marginfigure}
  ...
  \caption{Spectrogramme d'amplitude (a) et de phase (b) d'une note de piano.}
  \label{fig:phase}
\end{marginfigure}
...
Dans notre présentation des algorithmes d'estimation des paramètres long terme, on supposera que le signal analysé est composé d'une ou plusieurs sources dont une grande partie de l'énergie peut être correctement approximée par ce modèle. On fera également état ici des approches dites de \og suivi \fg de partiels, qui considèrent connu un modèle court terme du signal analysé. Le but d'un algorithme de suivi de partiel est en quelque sorte de restaurer la continuité temporelle entre les composantes à court terme d'un même partiel.
...
\label{eq:macseuil}
\end{equation}
où le seuil en fréquence $\Delta_{f}$ est un paramètre du modèle, alors le partiel $\mathcal{P}_{p}$ est prolongé à trame $t+1$. Dans un principe d'allocation exclusif, l'atome $p_{t+1, c_{\mathrm{min}}}$ ne pourra pas être utilisé pour la continuation d'un autre partiel. Chaque atome non utilisé à la trame $t+1$ devient alors une composante long terme qui sera potentiellement prolongée aves des atomes de la trame $t+2$.
...
\end{center}
"Diffusion maps" obtenues à partir du scattering d'ondelettes à l'ordre 1 et 2.
}
